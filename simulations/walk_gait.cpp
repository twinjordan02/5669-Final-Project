...
{
    // Initialize Gazebo and setup ROS nodes
    ...
    // Setup servo_pub as a Publisher and assign specific 'channels' to each index
    ...
    // Make the robot stand up from the ground home position
    motion_init();

    // Wait one second before starting to walk
    usleep(1000000); 

    // Call the function to generate the gait data which updates gaitTraj
    // (add in gait.h to this file)   
    // modify forward_walk() to reference gaitTraj or take it as an argument
