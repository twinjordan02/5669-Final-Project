  ...
  if (motiontime > (STAND_TIME_MS) && motiontime < GAIT_DURATION_MS) {
    ...
    if (motiontime % timingDelay == 0) {
     ...
     // }
      ...
      // PD(FR_0, -0.1, K_hip, D);
      // PD(FL_0, 0.1, K_hip, D);
      // The knees need extra strength for this gait
      ...
      PD(FR_2, gaitTrajs[gait_num].legs[RF].q3[cycle_iter], K_front + 30, D);     
      PD(FL_1, gaitTrajs[gait_num].legs[LF].q2[cycle_iter], K_front, D);
      PD(FL_2, gaitTrajs[gait_num].legs[LF].q3[cycle_iter], K_front + 30, D);
      PD(RR_1, gaitTrajs[gait_num].legs[RR].q2[cycle_iter], K_rear, D);
      PD(RR_2, gaitTrajs[gait_num].legs[RR].q3[cycle_iter], K_rear + 30, D);
      PD(RL_1, gaitTrajs[gait_num].legs[LR].q2[cycle_iter], K_rear, D);
      PD(RL_2, gaitTrajs[gait_num].legs[LR].q3[cycle_iter], K_rear + 30, D);
      gait_counter++;
    }
  }
...
int main(void)
{
  ...
  int gait_num = 2; // 0 = walk, 1 = trot, 2 = gallop, 3 = canter
